...
def pops_main(config_dict):
    ### --------------------------------- Basic settings --------------------------------- ###
    ### --------------------------------- Logging --------------------------------- ###
    # Remove any previous handlers (important)
    for h in logging.root.handlers[:]:
        logging.root.removeHandler(h)

    # Define log file path based on PoPS output prefix
    log_file = config_dict["out_prefix"] + ".log"

    # Create handlers: print to screen + save to file
    handlers = [
