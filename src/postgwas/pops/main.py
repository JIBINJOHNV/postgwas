...
from postgwas.gene_tests.magma_main import run_magma_analysis
...
def run_pops_direct(args: argparse.Namespace) -> None:
    ...
    # --------------------------------------------
    pops_folder = Path(args.output_folder)
    pops_folder.mkdir(parents=True, exist_ok=True)

    pops_out_prefix = pops_folder / f"{args.sample_name}_pops"
    ...
    pops_argv = [
        ...
        "--magma_prefix", args.magma_prefix,
...
def run_pipeline_logic(args):
    ...
    magma_inputs = vcf_to_magma(
        ...
        output_folder=f"{args.output_folder}/magma_inputs/",
    ...
    magma_folder = f"{args.output_folder}/magma_analysis/"
    Path(magma_folder).mkdir(parents=True, exist_ok=True)

    # ---------- Step 2: Run MAGMA ----------
    print("\n📌 Step 2 — Running MAGMA...")
    log_file = str(Path(magma_folder) / f"{args.sample_name}.magma.log")
    ...
    pops_folder = f"{args.output_folder}/"
